...
import { fetchSlidesQuestions, getEventAnswers, getRecomendationsById, submitAnswers } from "./api";
...
const DecisionPage: FunctionComponent<DecisionPageProps> = ({ resumeEvent }) => {
    ...
    useEffect(() => {
        const loadResumeState = async () => {
            ...
            setSlidersQuestions(questions);
            const mappedAnswers = (answers || []).reduce((acc: Answers, item: any) => ({
                ...acc,
    ...
    const steps: Record<DecisionStep, ReactElement> = {
        ...
        sliding: isGeneratingRecommendation || isResuming ? (
            loadingScreen
        ) : (
            <Slider questions={slidersQuestions} handleAnswers={handleAnswers} initialAnswers={existingAnswers} />
        ),
        recommendation: isResuming || isGeneratingRecommendation ? (
            loadingScreen
        ) : (
            <RecommendationsPage
                eventId={eventId}
