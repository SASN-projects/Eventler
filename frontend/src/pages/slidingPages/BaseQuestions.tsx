...
export const BaseQuestions: FunctionComponent<BaseQuestionsProps> = ({ onBaseComplete: onComplete }) => {
    ...
    const [groups, setGroups] = useState<Array<{ id: string; name: string; members?: any[] }>>([]);
    ...
    const handleDecisionModeSelect = async (mode: 'solo' | 'group') => {
        ...
        const id = await postNewEvent(baseParams.time, baseParams.place, baseParams.participantsAmount, 'individual');
        onComplete(id);
    };
                ...
                <DialogContent>
                    {groupsLoading ? (
                        ...
                        <Typography sx={{ py: 3, textAlign: 'center' }}>
                            You are not a member of any groups yet. Create or join one first to continue with group decision.
                        </Typography>
                    ) : (
                        <List>
                            {groups.map((group) => (
                                ...
                                >
                                    <ListItemText
                                        primary={group.name}
                                        secondary={group.members?.length ? `${group.members.length} members` : 'No members'}
                                    />
                                </ListItemButton>
                            ))}
                        </List>
